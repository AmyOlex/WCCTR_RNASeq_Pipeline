...
cat << EOF
usage: $0 options
This script is used on the Cell Ranger count output and must be run from the outs directory.  It splits the human and mouse barcodes from the GEM analysis, filters the BAM file to mouse reads, sorts the BAM file and then exports to 2 Fastq files.
...
EXAMPLE USAGE:
   >> ~/03_create_mouse_bam.sh -f samples.list  -d /path/to/results/dir/
...
do
	...
	subset-bam --bam $bampath/possorted_genome_bam.bam --cell-barcodes $gempath/mouse_barcodes.txt --out-bam $bampath/mouse_genome_bam.bam --cores 47

	echo Converting BAM to Fastq

	bamtofastq $bampath/mouse_genome_bam.bam $DIR/$sampleid
