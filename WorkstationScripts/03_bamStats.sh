...
OPTIONAL ARGUMENTS:
	-d		Directory of where results should be saved.  Sub-folder 03_bamstats will be created here.
...
EXAMPLE USAGE:
   >> ~/03_bamStats.sh -f /path/to/inventoryFile.list -d /path/to/results/dir/
...
OUTDIR=$DIR/03_bamstats

cat $INPUT | while read file
do 
	echo $file;
	samtools stats -d $file > $file.stats
done
